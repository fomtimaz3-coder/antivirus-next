...
self.onmessage=async({data})=>{
 ...
 const inspect=async(b,name)=>{const result=staticAnalysis(b,name),y=await yaraScan(new Blob([b]),rulePack?.source);result.yara=y;if(y.error){result.partial=true;result.limitations.push(y.error)}else for(const rule of y.matches||[])result.findings.push({rule,title:rule==='eicar'?'Обнаружен безвредный тест EICAR':'Совпадение YARA: '+rule,kind:rule==='eicar'?'test':'yara'});return result};
 try{
 ...
 if(cloud){progress('reputation');reputation=await new Promise(resolve=>{cloudResolve=resolve;self.postMessage({kind:'hash-ready',id,sha256})});if(reputation.status==='detections'){found.push({rule:'VT-REPUTATION',title:'VirusTotal: есть обнаружения',kind:'reputation',evidence:reputation.stats});self.postMessage({kind:'alert',id,text:'VirusTotal сообщил об обнаружениях; продолжаем локальную проверку'})}if(reputation.status==='unavailable')limitations.push('Репутация не проверена: '+reputation.reason);log.push('VirusTotal: '+reputation.status+'; локальный анализ не пропускается')}
 progress('yara');yara=await yaraScan(file,rulePack?.source);if(yara.error)limitations.push(yara.error);else for(const rule of yara.matches||[])found.push({rule,title:rule==='eicar'?'Обнаружен безвредный тест EICAR':'Совпадение YARA: '+rule,kind:rule==='eicar'?'test':'yara'});
 ...
 const head=new Uint8Array(await file.slice(0,8).arrayBuffer());let type=analysis?.details?.format||'Не определён';if(head[0]===0x50&&head[1]===0x4b){type='ZIP / APK';progress('archive');try{archive=await inspectZip(file,()=>[],p=>progress('archive',p),inspect);for(const entry of archive.entries){for(const finding of entry.findings)found.push({...finding,path:entry.name});const h=index.find(entry.sha256);if(h)found.push({rule:'HASH-DB-INNER',title:h.name,kind:'hash',path:entry.name})}limitations.push(...archive.limitations);if(archive.skipped)limitations.push('Пропущено записей ZIP: '+archive.skipped)}catch(e){archive={entries:[],error:e.message,scanned:0,skipped:0,limitations:[e.message]};limitations.push('ZIP: '+e.message)}}
 ...
 limitations.push('Стартовый набор YARA: EICAR, WannaCry, Petya. Это не полная актуальная база угроз.','Статический анализ не доказывает безопасность и не проверяет поведение файла.');
