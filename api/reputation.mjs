...
export default async function handler(req,res){
 ...
 if(r.status===404&&provider==='VirusTotal')return res.status(200).json({...common,status:'unknown'});if(r.status===429)return res.status(429).json({error:'quota'});if(!r.ok)return res.status([401,403].includes(r.status)?401:502).json({error:'provider'});
 ...
 const a=data.data?.attributes,s=a?.last_analysis_stats;if(!s)return res.status(502).json({error:'invalid_report'});const stats={};for(const k of ['malicious','suspicious','undetected','harmless']){if(!Number.isInteger(s[k])||s[k]<0)return res.status(502).json({error:'invalid_stats'});stats[k]=s[k]}
 return res.status(200).json({...common,status:stats.malicious||stats.suspicious?'detections':'no_detections',analysisAt:a.last_analysis_date||null,stats});
