...
report=await scan(zip([['first.txt',eicar],['second.txt',eicar]]),'duplicates.zip');assert(report.findings.some(x=>x.rule==='eicar'&&x.path==='first.txt'));assert(report.findings.some(x=>x.rule==='eicar'&&x.path==='second.txt'));assert(report.log.some(x=>x.endsWith('SHA-256: 1')));
